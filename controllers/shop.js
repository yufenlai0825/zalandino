...
exports.getCheckout = (req, res, next) => {
  ...
  .then(user => {
      ...
      return stripe.checkout.sessions.create({
        ...
        client_reference_id: req.user._id.toString() // client key that stores user ID for webhook verification logic
      })
    })
  .then(session => {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    console.log("Session:", session.id); 
      ...
      try {
        ...
        return res.status(500).send('Error occurred while processing the webhook.');
      }
  }
...
exports.getInvoice = (req, res, next) => {
  ...
  order.items.forEach(item => {
    ...
    pdfDoc.fontSize(20).text(item.productData.title + " - " + item.qty + " * " + "$" + item.productData.price); 
  });
